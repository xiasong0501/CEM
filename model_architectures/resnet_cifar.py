...
class CifarResNet(nn.Module):
  ...
  """
  def __init__(self, block, depth, cutting_layer, logger, num_client = 1, num_class = 10, initialize_different = False, adds_bottleneck = False, bottleneck_option = "C8S1", sgm=True,double_local_layer=False,upsize=False,SCA=False):
    """ Constructor
    Args:
    ...
    layers = []
    # if upsize:
    #   layers.append(conv5x5(3, 16,2))
    # else:
    layers.append(conv5x5(3, 16))
    if double_local_layer:
      layers.append(nn.Conv2d(16, 16, kernel_size=3, stride = 1, padding = 1, bias=False))
      cutting_layer=cutting_layer+1
    self.inplanes = 16
    self.stage_1 = self._make_layer(block, 32, layer_blocks, 2)
    layers.extend(self.stage_1)
    self.stage_2 = self._make_layer(block, 64, layer_blocks, 2)
    layers.extend(self.stage_2)
    self.stage_3 = self._make_layer(block, 128, layer_blocks, 2)
    ...
    if adds_bottleneck: # to enable gooseneck, simply copy below to other architecture
        ...
        if SCA:
            ...
            # lca_layer = LCALayer(input_shape=(8, 16, 16), num_neurons=2048, alpha=0.05, lmbda=0.01, num_iterations=1)
            local+=[lcapt_conv]
    ...
    self.classifier = nn.Linear(128*block.expansion, num_class)
    print("local:")
    print(self.local)
    print("cloud:")
    print(self.cloud)
...
def ResNet20(cutting_layer, logger, num_client = 1, num_class = 10, initialize_different = False, adds_bottleneck = False, bottleneck_option = "C8S1",double_local_layer=False,upsize=False,SCA=False):
  """Constructs a ResNet-20 model for CIFAR-10 (by default)
  Args:
    num_classes (uint): number of classes
  """
  model = CifarResNet(ResNetBasicblock, 20, cutting_layer, logger, num_client = num_client, num_class = num_class, initialize_different = initialize_different, adds_bottleneck = adds_bottleneck, bottleneck_option = bottleneck_option,double_local_layer=double_local_layer,upsize=upsize,SCA=SCA)
  return model

def ResNet32(cutting_layer, logger, num_client = 1, num_class = 10, initialize_different = False, adds_bottleneck = False, bottleneck_option = "C8S1",double_local_layer=False,upsize=False,SCA=False):
  """Constructs a ResNet-32 model for CIFAR-10 (by default)
  Args:
    num_classes (uint): number of classes
  """
  model = CifarResNet(ResNetBasicblock, 32, cutting_layer, logger, num_client = num_client, num_class = num_class, initialize_different = initialize_different, adds_bottleneck = adds_bottleneck, bottleneck_option = bottleneck_option,double_local_layer=double_local_layer,upsize=upsize,SCA=SCA)
  return model

def ResNet44(cutting_layer, logger, num_client = 1, num_class = 10, initialize_different = False, adds_bottleneck = False, bottleneck_option = "C8S1",double_local_layer=False,upsize=False,SCA=False):
  """Constructs a ResNet-44 model for CIFAR-10 (by default)
  Args:
    num_classes (uint): number of classes
  """
  model = CifarResNet(ResNetBasicblock, 44, cutting_layer, logger, num_client = num_client, num_class = num_class, initialize_different = initialize_different, adds_bottleneck = adds_bottleneck, bottleneck_option = bottleneck_option,double_local_layer=double_local_layer,upsize=upsize,SCA=SCA)
  return model

def ResNet56(cutting_layer, logger, num_client = 1, num_class = 10, initialize_different = False, adds_bottleneck = False, bottleneck_option = "C8S1",double_local_layer=False,upsize=False,SCA=False):
  """Constructs a ResNet-56 model for CIFAR-10 (by default)
  Args:
    num_classes (uint): number of classes
  """
  model = CifarResNet(ResNetBasicblock, 56, cutting_layer, logger, num_client = num_client, num_class = num_class, initialize_different = initialize_different, adds_bottleneck = adds_bottleneck, bottleneck_option = bottleneck_option,double_local_layer=double_local_layer,upsize=upsize,SCA=SCA)
  return model

def ResNet110(cutting_layer, logger, num_client = 1, num_class = 10, initialize_different = False, adds_bottleneck = False, bottleneck_option = "C8S1",double_local_layer=False,upsize=False,SCA=False):
  """Constructs a ResNet-110 model for CIFAR-10 (by default)
  Args:
    num_classes (uint): number of classes
  """
  model = CifarResNet(ResNetBasicblock, 110, cutting_layer, logger, num_client = num_client, num_class = num_class, initialize_different = initialize_different, adds_bottleneck = adds_bottleneck, bottleneck_option = bottleneck_option,double_local_layer=double_local_layer,upsize=upsize,SCA=SCA)
  return model
